...
contract Joker is ERC20, Ownable {
    ...
    // Event to track minting of new tokens
    event TokensMinted(address indexed to, uint256 amount);

    // Event to track burning of tokens
    event TokensBurned(address indexed from, uint256 amount);

    // Constructor that mints the initial supply to the deployer's address and sets the owner
    constructor(uint256 initialSupply) ERC20("JOKERZ", "JKRZ") Ownable(msg.sender) {
    ...
    function mint(address to, uint256 amount) public onlyOwner {
        ...
        _mint(to, amount); // Call internal _mint function
        emit TokensMinted(to, amount); // Emit event to track minting
    }

    // Function to burn tokens, restricted to the contract owner
    function burn(address from, uint256 amount) public onlyOwner {
        require(from != address(0), "ERC20: burn from the zero address");
        _burn(from, amount); // Call internal _burn function
        emit TokensBurned(from, amount); // Emit event to track burning
    }
